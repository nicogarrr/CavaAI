...
export default async function FinancialTerminalPage({ params, searchParams }: { params: Promise<{ ticker: string }>; searchParams: Promise<{ metrics?: string; years?: string; periodicity?: string }> }) {
  ...
  return <main id="content" tabIndex={-1} className="mx-auto flex w-full max-w-[1600px] flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <PageHeader back={<Button asChild size="sm" variant="ghost"><Link href={`/research/${ticker}`}><ArrowLeft className="h-4 w-4" />Workspace de {ticker}</Link></Button>} description="Métricas reportadas, normalizadas y calculadas con definiciones, fórmulas, confianza y trazabilidad de la fuente." kicker="Financieros con fuentes" title="Terminal financiero" />
