...
function modelMoney(value: number | null | undefined) {
  return value == null || !Number.isFinite(value) ? 'N/A' : compactMoney(value);
}

function modelPct(value: number | null | undefined) {
  return value == null || !Number.isFinite(value) ? 'N/A' : pct(value);
}

function LongTermModelPanel({ model }: { model: ResearchLongTermModel | null }) {
  if (!model) {
    return (
  ...
  const scenarioRows = Object.entries(model.scenarios);
  ...
  return (
    <section className="rounded-lg border border-gray-800 bg-[#111111] p-5">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <Stat label={`${year5?.year ?? '5Y'} Revenue`} value={modelMoney(year5?.revenue)} />
        <Stat label={`${year5?.year ?? '5Y'} FCF`} value={modelMoney(year5?.free_cash_flow)} tone="good" />
        <Stat label="FCF margin" value={modelPct(year5?.fcf_margin)} />
        <Stat label="FCF / share" value={modelMoney(year5?.fcf_per_share)} tone="good" />
      </div>
        ...
        <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] text-left text-sm">
              ...
              <tbody>
                {scenarioRows.map(([name, scenario]) => {
                  ...
                  return (
                    <tr key={name} className="border-b border-gray-900 last:border-0">
                      ...
                      <td className="py-3 text-right text-gray-300">{modelMoney(point?.revenue)}</td>
                      <td className="py-3 text-right text-gray-300">{modelMoney(point?.free_cash_flow)}</td>
                      <td className="py-3 text-right text-gray-300">{modelPct(point?.fcf_margin)}</td>
                      <td className="py-3 text-right text-teal-200">{modelMoney(scenario.valuation?.value_per_share)}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div className="space-y-3">
          <div className="rounded-md border border-gray-800 p-3">
            ...
            <div className="mt-2 grid gap-2 text-sm">
              <div className="flex justify-between gap-3"><span className="text-gray-400">Revenue CAGR</span><span className="text-gray-200">{modelPct(growthAssumption?.value)}</span></div>
              <div className="flex justify-between gap-3"><span className="text-gray-400">Normalized FCF margin</span><span className="text-gray-200">{modelPct(marginAssumption?.value)}</span></div>
              <div className="flex justify-between gap-3"><span className="text-gray-400">ROIC / WACC</span><span className="text-gray-200">{modelPct(terminal?.roic)} / {modelPct(model.assumptions.wacc?.value)}</span></div>
            </div>
          ...
          <div className="rounded-md border border-gray-800 p-3 text-sm">
            ...
            <p className="mt-2 text-gray-300">
              {model.reverse_dcf.status === 'ok'
                ? `El precio actual exige ${modelPct(model.reverse_dcf.required_revenue_growth)} de crecimiento; la base asume ${modelPct(model.reverse_dcf.base_revenue_growth)}.`
      ...
      <div className="mt-5 grid gap-6 xl:grid-cols-3">
        <div className="rounded-md border border-gray-800 p-3 text-sm">
          ...
          <p className="mt-2 text-gray-400">Revenue CAGR {modelPct(model.quality_of_growth.revenue_cagr.value)} · FCF margin change {modelPct(model.quality_of_growth.fcf_margin_change.value)}</p>
        ...
        <div className="rounded-md border border-gray-800 p-3 text-sm">
          ...
          <div className="mt-2 text-xl font-semibold text-gray-200">{modelMoney(model.owner_earnings.value)}</div>
        ...
        <div className="rounded-md border border-gray-800 p-3 text-sm">
          ...
          <p className="mt-2 text-gray-400">Top-down TAM {modelMoney(model.market_opportunity.top_down.tam.value)} · bottom-up {modelMoney(model.market_opportunity.bottom_up.value)}</p>
