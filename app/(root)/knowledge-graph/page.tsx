...
export default async function KnowledgeGraphPage({ searchParams }: { searchParams: Promise<{ node_types?: string; ticker?: string; limit?: string; node?: string; depth?: string }> }) {
  ...
  const typeCounts = Object.entries(graph.nodes.reduce<Record<string, number>>((acc, node) => ({ ...acc, [node.type]: (acc[node.type] ?? 0) + 1 }), {})).sort((a, b) => b[1] - a[1]);
    return <main id="content" tabIndex={-1} className="mx-auto flex w-full min-w-0 max-w-[1500px] flex-col gap-6 overflow-x-clip"><PageHeader actions={<MutationForm action={syncKnowledgeGraph} successMessage="Grafo sincronizado"><Button className="h-11 w-full md:w-auto" type="submit"><RefreshCcw className="h-4 w-4" />Sincronizar grafo</Button></MutationForm>} description="Explora enlaces deterministas entre autores, principios, empresas, KPIs, riesgos, decisiones, lecciones y conceptos." kicker="Research conectado" title="Grafo de conocimiento" />
